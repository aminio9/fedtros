...
`src.utils.utils.set_seed` seeds Python, NumPy, PyTorch, and CUDA. Relevant config keys:
...
`runtime=tiny` and `runtime=cpu` are CPU-only. `runtime=full` is a GPU-scale overlay and should not be used for preflight validation.

## Data And Leakage Controls

Preprocessing fits scaler/encoder objects on known training rows only. Unknown
samples are excluded from known training and validation tensors and are appended
only to open-set test tensors.

EVT fitting and threshold calibration use known validation samples only. Test data must not be used for checkpoint selection, threshold selection, early stopping, or post-hoc tuning.

## Traceability

Each run should be traceable through:

- `resolved_config.yaml`
- `metadata.json`
- `metrics.jsonl` and `metrics.csv`
- checkpoint files and `checkpoint_metadata.json`
- `federated_history.csv` for federated runs
- method-specific monitoring JSONL files
- evaluation JSON/CSV outputs
- `plots/plot_manifest.json`

`docs/source_code_map.md` records the current canonical entry points, call chains, and unsupported experimental scaffolds.
...
If these fail, do not run full experiments.

Validation intentionally fails for non-EVT scorer configs when they are combined with `run.py` experiment pipelines; those scorer components are implemented utilities, not completed experiment routes.

## Full Experiments

The shell scripts in `scripts/experiments/` are command files for paper-scale runs. `run_full_suite.sh` and `e7_efficiency_scalability.sh` are intentionally expensive. Full results require repeated seeds and matched baselines before claims are made.
