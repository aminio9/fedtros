...
class FlowerClient(fl.client.NumPyClient):
    ...
    def _configure_local_class_support(self) -> None:
        ...
        try:
            hist = json.loads(str(self.local_data_profile.get("label_histogram", "{}")))
        except Exception:
            self.logger.warning("Client %s: failed to parse local label histogram.", self.cid)
            hist = {}
        num_actions = int(self.cfg.model.num_actions)
        counts = [int(hist.get(str(i), 0)) for i in range(num_actions)]
        self.agent.set_local_class_counts(counts)
        if hasattr(self.policy, "set_allowed_actions"):
            self.policy.set_allowed_actions([i for i, count in enumerate(counts) if count > 0])
        self.logger.info(
            "Client %s local class support configured | coverage=%d/%d | counts=%s",
            self.cid,
    ...
    def _init_closed_set_evaluation(self) -> None:
        ...
        if not (candidate_paths and class_names_rel):
        ...
        selected: tuple[str, Path, torch.Tensor, torch.Tensor] | None = None

        for eval_role, eval_data_rel in candidate_paths:
            eval_data_path = _resolve_project_path(eval_data_rel)
            if not eval_data_path.exists():
                continue
            try:
                data = torch.load(eval_data_path, map_location="cpu", weights_only=True)
                features = data["features"].to(device=data_device).float()
                labels = data["labels"].to(device=data_device).long()
            except Exception as exc:
                self.logger.error(
                    "Client %s: failed to load closed-set %s data from %s: %s",
                    self.cid,
                    eval_role,
                    eval_data_path,
                    exc,
            ...
            if int(features.shape[0]) != num_samples:
                self.logger.warning(
                    "Client %s: closed-set %s data %s has mismatched features/labels "
                    "(features=%d, labels=%d); trying next candidate.",
                    self.cid,
                    eval_role,
                    eval_data_path.name,
            ...
            if num_samples == 0:
                self.logger.warning(
                    "Client %s: closed-set %s data %s has zero samples; trying next candidate.",
                    self.cid,
                    eval_role,
                    eval_data_path.name,
                )
            ...
            selected = (eval_role, eval_data_path, features, labels)
            break

        if selected is None:
            self.logger.warning("Client %s: no non-empty closed-set eval data found.", self.cid)
            return
    ...
    def _evaluate_closed_set(
        ...
        metrics = {
            ...
            "balanced_accuracy": balanced_accuracy,
            "f1_macro": f1,
            "per_class_recall": json.dumps(per_class_recall, sort_keys=True),
            "per_class_f1": json.dumps(per_class_f1, sort_keys=True),
            "num_examples": num_examples,
            "eval_dataset_role": self.eval_dataset_role,
        }
        if self.eval_dataset_role == "validation":
            metrics.update(
                {
                    "val/accuracy": accuracy,
                    "val/balanced_accuracy": balanced_accuracy,
                    "val/macro_f1": f1,
                    "val/num_examples": num_examples,
                }
            )

        return loss, num_examples, metrics

    def _load_evt_calibration_data(
        self,
        fallback_features: torch.Tensor,
        fallback_labels: torch.Tensor,
    ) -> tuple[torch.Tensor, torch.Tensor, str]:
        """Prefer the validation tensor for EVT calibration during FL evaluation."""
        validation_rel = getattr(getattr(self.cfg, "evaluation", None), "validation_data", None)
        unknown_label_id = int(getattr(self.cfg.evt, "unknown_label_id", -1))
        calibration_protocol = str(
            getattr(self.cfg.evt, "calibration_protocol", "validation_only")
        ).lower()
        if validation_rel:
            validation_path = _resolve_project_path(validation_rel)
            if validation_path.exists():
                try:
                    data = torch.load(validation_path, map_location="cpu", weights_only=True)
                    features = data["features"].float()
                    labels = data["labels"].long()
                    known_mask = labels != unknown_label_id
                    if not bool(known_mask.all()):
                        dropped = int((~known_mask).sum().item())
                        self.logger.warning(
                            "Client %s: validation EVT calibration contained %d unknown labels; "
                            "dropping them.",
                            self.cid,
                            dropped,
                        )
                    ...
                    if labels.numel() > 0:
                        self.logger.info(
                            "Client %s: using validation tensor for EVT calibration "
                            "(%s, samples=%d).",
                            self.cid,
                            validation_path.name,
                            int(labels.numel()),
                        )
                        return features, labels, str(validation_path)
                    self.logger.warning(
                        "Client %s: validation EVT calibration tensor is empty after filtering.",
                        self.cid,
                    )
                except Exception:
                    self.logger.exception(
                        "Client %s: failed to load validation EVT calibration tensor %s.",
                        self.cid,
                        validation_path,
                    )
        ...
        if calibration_protocol == "validation_only":
            raise FileNotFoundError(
                "EVT calibration_protocol=validation_only requires an existing validation tensor "
                "at evaluation.validation_data."
            )

        self.logger.warning(
            "Client %s: falling back to local training shard for EVT calibration; "
            "round-level open-set metrics are not validation-protocol metrics.",
            self.cid,
        )
    ...
    def _fit_evt_and_run_openset_eval(
        ...
        try:
            (
                ...
                calibration_labels,
                calibration_source,
            ) = self._load_evt_calibration_data(features, labels)
        except Exception:
            self.logger.exception("Client %s: EVT calibration data loading failed.", self.cid)
            return {}
