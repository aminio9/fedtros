...
The historical ``prototype_rank`` name now carries the calibrated robust
PNPFF-Fed unknown score. The raw Equation 27-28 score is retained in exported
diagnostics. Other components retain rank calibration and can still be combined
with PNPFF by the mean, weighted, and maximum fusion modes.
"""
...
def _rank_high(value: float, sorted_values: np.ndarray) -> float:
    ...
    if not np.isfinite(value) or sorted_values.size == 0:
        return float("nan")
    return float(
        np.searchsorted(sorted_values, float(value), side="right") / max(sorted_values.size, 1)
    )
...
def _fuse_rank_scores(
    proser_rank: float, gen_rank: float, energy_rank: float, proto_rank: float, cfg: Any
) -> float:
...
class PrototypeBank:
    ...
    def score(self, features: np.ndarray, class_id: int) -> np.ndarray:
        ...
        neg_close = np.maximum(
            0.0,
            (float(self.negative_radius) - neg_dist) / max(float(self.negative_radius), self.eps),
        )
    ...
    def to_payload(self) -> dict[str, Any]:
        return {
            ...
            "negative_prototypes": self.negative_prototypes.tolist()
    ...
    *,
    ...
    with torch.no_grad():
        ...
        for x, y in loader:
            ...
            if (
                bool(getattr(student_model, "open_set_enabled", False))
                and getattr(student_model, "open_set_head", None) is not None
            ):
                ...
                rows.append(
                    {
                        ...
                        "correct_known": int(
                            (y[i].item() >= 0) and (y[i].item() == pred[i].item())
                        ),
...
def _fit_prototypes(
    calib_df: pd.DataFrame, num_classes: int, cfg: Any, log: logging.Logger
) -> PrototypeBank:
    ...
    for c in range(num_classes):
        ...
        if k <= 1:
            ...
            try:
                ...
                log.warning(
                    "Fed-DiGOS prototype KMeans failed for class=%d: %s; using mean", c, exc
                )
                centers = feats.mean(axis=0, keepdims=True)
    ...
    negative_radius = 1.0
    negative_weight = (
        float(_nested(cfg, "prototype.negative.weight", 0.35)) if negative_enabled else 0.0
    )
        ...
        if boundary.size > 0:
            ...
            if k_neg <= 1:
                ...
                try:
                    ...
                    neg_centers = boundary.mean(axis=0, keepdims=True)
            neg_centers = (
                _l2_normalize_np(neg_centers) if normalize else neg_centers.astype(np.float64)
            )
            ...
            negative_radius = float(
                max(np.quantile(nearest_neg, min(max(neg_q, 0.50), 0.999)), EPS)
            )
...
def _class_calibration_slice(
    calib_df: pd.DataFrame, class_id: int, cfg: Any, log: logging.Logger
) -> pd.DataFrame:
...
def _build_rank_calibrators(
    ...
    *,
    ...
    def build_one(class_key: int, cls_fit: pd.DataFrame) -> None:
        if len(cls_fit) < min_errors:
            log.warning(
                "Fed-DiGOS rank calibration skipped class=%s | samples=%d min=%d",
                class_key,
                len(cls_fit),
                min_errors,
            )
        ...
        if generator_col not in cls_fit.columns:
            log.warning(
                "Fed-DiGOS rank calibration requested missing generator score column=%s; falling back to gen_score",
                generator_col,
            )
            gen_col = "gen_score"
        else:
        ...
        proser_values = (
            np.sort(_finite_array(cls_fit["proser_score"].to_numpy()))
            if "proser_score" in cls_fit.columns
            else np.asarray([], dtype=np.float64)
        )
        proto_values = np.sort(_finite_array(cls_fit["prototype_score"].to_numpy()))
        pnpff_active = (
            "pnpff_threshold" in cls_fit.columns
            and "pnpff_healthy" in cls_fit.columns
            and bool(cls_fit["pnpff_healthy"].iloc[0])
        )
        ...
        q75, q25 = (
            np.quantile(energy_values, [0.75, 0.25])
            if energy_values.size
            else np.asarray([1.0, 0.0])
        )
        energy_iqr = float(max(q75 - q25, EPS))
        energy_devs = (
            np.sort(np.abs(energy_values - energy_median) / energy_iqr)
            if energy_values.size
            else np.asarray([], dtype=np.float64)
        )
        neg_energy_values = (
            np.sort(-energy_values) if energy_values.size else np.asarray([], dtype=np.float64)
        )
        ...
        for _, row in cls_fit.iterrows():
            ...
            proto_rank = float(row["prototype_score"]) if pnpff_active else float("nan")
            ...
            fused_known.append(
                _fuse_rank_scores(proser_rank, gen_rank, energy_rank, proto_rank, cfg)
            )
        ...
        log.info(
            ...
            gen_col,
            _safe_quantile(cls_fit["proser_score"], 0.50)
            if "proser_score" in cls_fit.columns
            else float("nan"),
...
def _score_with_rank_calibrator(row: pd.Series, calibrator: dict[str, Any]) -> dict[str, float]:
    ...
    if gen_col not in row.index:
        gen_col = "gen_score"
    proser_rank = _rank_high(
        float(row.get("proser_score", 0.0)),
        calibrator.get("proser_values", np.asarray([], dtype=np.float64)),
    )
    ...
    energy_dev, energy_rank = _energy_rank(
        ...
        sorted_deviations=calibrator.get(
            "energy_deviation_values", np.asarray([], dtype=np.float64)
        ),
    ...
    return {
        ...
        "mean_threshold": float(
            calibrator.get("mean_threshold", calibrator.get("fusion_threshold", 1.0))
        ),
        "weighted_threshold": float(
            calibrator.get("weighted_threshold", calibrator.get("fusion_threshold", 1.0))
        ),
        "max_threshold": float(
            calibrator.get("max_threshold", calibrator.get("fusion_threshold", 1.0))
        ),
    }
...
def calibrate_fed_digos(
    ...
    *,
    ...
    if np.any(labels_np == unknown_label_id):
        raise ValueError(
            "Fed-DiGOS calibration data must contain known classes only; found unknown labels."
        )
    ...
    min_errors = int(_nested(cfg, "evt.min_errors_per_class", 50))
    known_labels = torch.from_numpy(df["y_raw"].to_numpy(dtype=np.int64, copy=True)).long()
    pnpff_cfg = PNPFFConfig.from_cfg(
        cfg,
        feature_dim=int(student_model.feature_dim),
    ...
    pnpff_detector = fit_pnpff_detector(
        ...
        features[pnpff_calibration_idx],
        known_labels[pnpff_calibration_idx],
        num_classes=num_classes,
        cfg=pnpff_cfg,
        device=device,
        backbone=student_model.backbone,
        backbone_feature_dim=int(student_model.feature_dim),
    )
    pnpff_scores = pnpff_detector.predict_inputs(features, device=device)
    df["prototype_score"] = pnpff_scores["unknown_score"].astype(float)
    df["pnpff_raw_unknown_score"] = pnpff_scores["raw_unknown_score"].astype(float)
    ...
    df["pnpff_embedding_norm"] = pnpff_scores["embedding_norm"].astype(float)
    df["pnpff_threshold"] = float(pnpff_detector.threshold)
    df["pnpff_healthy"] = bool(pnpff_detector.is_healthy)

    models: dict[str, dict[int, EVTModel]] = {
        "proser": {},
        "gen": {},
        "energy": {},
        "prototype": {},
    ...
    for c in range(num_classes):
        ...
        if len(cls_fit) < min_errors:
            log.warning(
                "Fed-DiGOS EVT skipped class=%d | samples=%d min=%d", c, len(cls_fit), min_errors
            )
            continue
        energy_values = _finite_array(cls_fit["energy_score"].to_numpy())
        energy_median = float(np.median(energy_values)) if energy_values.size else 0.0
        ...
        energy_iqr = float(max(q75 - q25, EPS))
        energy_deviation = (
            np.abs(energy_values - energy_median) / energy_iqr
            if energy_values.size
            else np.asarray([], dtype=np.float64)
        )
        score_specs = [
        ...
        log.info(
            ...
            float(
                models["energy_deviation"].get(c).decision_threshold
                if c in models["energy_deviation"]
                else np.nan
            ),
            _safe_quantile(cls_fit["prototype_score"], 0.95),
            float(
                models["prototype"].get(c).decision_threshold
                if c in models["prototype"]
                else np.nan
            ),
        )
...
def evaluate_fed_digos(
    ...
    *,
    ...
    open_set_label_id = int(_nested(cfg, "open_set_label_id", OPEN_SET_LABEL_ID))
    unknown_label_id = int(_nested(cfg, "unknown_label_id", UNKNOWN_LABEL_ID))
    selected_method = str(_nested(cfg, "score_fusion.method", "proser_rank")).lower()
    prototype_only = selected_method in {
        "prototype_rank",
        "proto_rank",
        "prototype",
        "proto_only",
    ...
    if not pnpff_detector.is_healthy and prototype_only:
        message = "PNPFF calibration health check failed: " + ", ".join(
            str(v) for v in pnpff_detector.health.get("reasons", ["unknown_reason"])
        )
        action = str(getattr(pnpff_detector.config, "health_action", "error")).lower()
        if action == "error":
            raise RuntimeError(message)
        log.warning(message)
    elif not pnpff_detector.is_healthy:
        log.warning(
            "PNPFF component disabled for combined fusion because calibration health failed: %s",
            pnpff_detector.health.get("reasons", []),
        )
    df = _collect_student_scores(
        features,
        labels,
    ...
    if calibration_df is not None and "rank_calibrators" in calibration_df.attrs:
        ...
        rank_calibrators = {}
        log.warning(
            "Fed-DiGOS evaluate has no calibration_df; rank-fusion scores will fall back to EVT probabilities."
        )
    ...
    for _, row in df.iterrows():
        ...
        for name, col in [
            ...
            if model is None or not np.isfinite(value):
                ...
                thresholds[name] = float(
                    model.decision_threshold if model.decision_threshold is not None else np.nan
                )
        ...
        if cal is not None:
            ...
            if not pnpff_detector.is_healthy and not prototype_only:
                proto_rank = float("nan")
            ...
            weighted_rank_score = _mean_finite(
                [proser_rank, gen_rank, energy_rank, proto_rank], weights=_score_weights(cfg)
            )
        ...
        if energy_dev_model is not None and np.isfinite(energy_dev):
            ...
            T_energy_dev = float(
                energy_dev_model.decision_threshold
                if energy_dev_model.decision_threshold is not None
                else np.nan
            )
        else:
    ...
    if np.unique(y_binary).size < 2:
        ...
        pd.DataFrame({"fpr": fpr, "tpr": tpr, "threshold": roc_thresholds}).to_csv(
            output_dir / "open_set_roc_curve.csv", index=False
        )
    ...
    known_acc_before = (
        float(accuracy_score(y_true[known_mask], y_before[known_mask])) if known_mask.any() else 0.0
    )
    known_acc_after = (
        float(accuracy_score(y_true[known_mask], y_pred[known_mask])) if known_mask.any() else 0.0
    )
    unknown_recall = (
        float(accuracy_score(y_true[unknown_mask], y_pred[unknown_mask]))
        if unknown_mask.any()
        else 0.0
    )
    known_false_unknown_rate = (
        float(np.mean(y_pred[known_mask] == open_set_label_id)) if known_mask.any() else 0.0
    )
    f1_macro = float(f1_score(y_true, y_pred, average="macro", zero_division=0))
    unknown_f1 = float(
        f1_score(y_binary, (y_pred == open_set_label_id).astype(int), zero_division=0)
    )
    ...
    df["selected_threshold_used"] = [
        row["T_proser_rank_used"]
        if str(selected_score_name).lower()
        in {"proser_rank", "placeholder_rank", "proser", "placeholder", "proser_only"}
        else row["T_gen_rank_used"]
        if str(selected_score_name).lower()
        in {"generator_rank", "gen_rank", "generator", "gen_only", "generator_only"}
        else row["T_energy_rank_used"]
        if str(selected_score_name).lower() in {"energy_rank", "energy", "energy_only"}
        else row["T_prototype_rank_used"]
        if str(selected_score_name).lower()
        in {"prototype_rank", "proto_rank", "prototype", "proto_only", "prototype_only"}
    ...
    if calibration_df is not None:
        calibration_df.drop(columns=["feature", "classifier_feature"], errors="ignore").to_csv(
            output_dir / "fed_digos_calibration_scores.csv", index=False
        )
    thresholds_payload = {
    ...
    (output_dir / "fed_digos_evt_thresholds.json").write_text(
        json.dumps(thresholds_payload, indent=2, sort_keys=True), encoding="utf-8"
    )
    (output_dir / "fed_digos_rank_calibration.json").write_text(
        json.dumps(rank_thresholds_payload, indent=2, sort_keys=True), encoding="utf-8"
    )
    pnpff_payload = pnpff_detector.to_payload()
    (output_dir / "fed_digos_prototypes.json").write_text(
        json.dumps(pnpff_payload, indent=2, sort_keys=True), encoding="utf-8"
    )
    (output_dir / "pnpff_metadata.json").write_text(
        json.dumps(pnpff_payload, indent=2, sort_keys=True), encoding="utf-8"
    )
    pnpff_detector.save(
        output_dir
        / str(getattr(getattr(cfg, "pnpff", None), "checkpoint_output", "pnpff_state.pt"))
    )
    ...
    quantile_cols = [
        "proser_score",
        "proser_unknown_prob",
        "proser_unknown_margin",
        "gen_score",
        "recon_error",
        "latent_nll",
        "energy_score",
        "energy_deviation_score",
        "prototype_score",
        "proser_rank_score",
        "gen_rank_score",
        "energy_rank_score",
    ...
    for col in quantile_cols:
        if col not in export_df.columns:
            continue
        known_vals = export_df.loc[export_df["known_or_unknown"] == "known", col].to_numpy(
            dtype=float
        )
        unk_vals = export_df.loc[export_df["known_or_unknown"] == "unknown", col].to_numpy(
            dtype=float
        )
        quantiles[col] = {}
    ...
    (output_dir / "known_unknown_score_quantiles.json").write_text(
        json.dumps(quantiles, indent=2, sort_keys=True), encoding="utf-8"
    )
    (output_dir / "score_overlap_report.json").write_text(
    ...
    raw_aurocs = {
        ...
        "auroc_proser_unknown_prob": _safe_auc(
            y_binary, export_df["proser_unknown_prob"].to_numpy(dtype=float)
        ),
        "auroc_proser_unknown_margin": _safe_auc(
            y_binary, export_df["proser_unknown_margin"].to_numpy(dtype=float)
        ),
        ...
        "auroc_prototype_score": _safe_auc(
            y_binary, export_df["prototype_score"].to_numpy(dtype=float)
        ),
        "auroc_energy_score_high": _safe_auc(
            y_binary, export_df["energy_score"].to_numpy(dtype=float)
        ),
        "auroc_energy_score_reversed": _safe_auc(
            y_binary, -export_df["energy_score"].to_numpy(dtype=float)
        ),
        "auroc_energy_deviation": _safe_auc(
            y_binary, export_df["energy_deviation_score"].to_numpy(dtype=float)
        ),
        "auroc_proser_rank": _safe_auc(
            y_binary, export_df["proser_rank_score"].to_numpy(dtype=float)
        ),
        "auroc_gen_rank": _safe_auc(y_binary, export_df["gen_rank_score"].to_numpy(dtype=float)),
        "auroc_energy_rank": _safe_auc(
            y_binary, export_df["energy_rank_score"].to_numpy(dtype=float)
        ),
        "auroc_prototype_rank": _safe_auc(
            y_binary, export_df["prototype_rank_score"].to_numpy(dtype=float)
        ),
    }
    ...
    def _component_decision_stats(name: str, reject_col: str) -> dict[str, float]:
        ...
        return {
            f"{name}_unknown_recall": float(np.mean(reject[unknown_mask]))
            if unknown_mask.any()
            else 0.0,
            f"{name}_known_false_unknown_rate": float(np.mean(reject[known_mask]))
            if known_mask.any()
            else 0.0,
            f"{name}_overall_acc": float(accuracy_score(y_true, pred_component)),
            f"{name}_rejected_total": float(np.sum(reject)),
            f"{name}_rejected_unknown": float(np.sum(reject[unknown_mask]))
            if unknown_mask.any()
            else 0.0,
            f"{name}_rejected_known": float(np.sum(reject[known_mask]))
            if known_mask.any()
            else 0.0,
        }
    ...
    component_decisions.update(
        _component_decision_stats("weighted_rank_fusion", "weighted_fusion_reject")
    )
    ...
    component_report = {
        **raw_aurocs,
        **component_decisions,
        "selected_score_name": str(selected_score_name),
    }
    (output_dir / "fed_digos_component_aurocs.json").write_text(
        json.dumps(component_report, indent=2, sort_keys=True), encoding="utf-8"
    )
    ...
    metrics = {
        ...
        "openset_rejected_unknown_by_fusion": float(
            np.sum(np.asarray(fusion_rejects)[unknown_mask])
        )
        if unknown_mask.any()
        else 0.0,
    ...
    log.info(
        ...
        unknown_recall,
        known_false_unknown_rate,
        unknown_as_normal_before,
        raw_aurocs["auroc_proser_rank"],
        raw_aurocs["auroc_gen_rank"],
        raw_aurocs["auroc_energy_rank"],
        raw_aurocs["auroc_prototype_rank"],
        raw_aurocs["auroc_mean_rank_fusion"],
        raw_aurocs["auroc_weighted_rank_fusion"],
        raw_aurocs["auroc_max_rank_fusion"],
        raw_aurocs["auroc_proser_score"],
        raw_aurocs["auroc_gen_score"],
        raw_aurocs["auroc_recon_error"],
        raw_aurocs["auroc_energy_score_high"],
        raw_aurocs["auroc_energy_score_reversed"],
        raw_aurocs["auroc_energy_deviation"],
        raw_aurocs["auroc_prototype_score"],
        component_decisions["proser_rank_unknown_recall"],
        component_decisions["proser_rank_known_false_unknown_rate"],
        component_decisions["gen_rank_unknown_recall"],
        component_decisions["gen_rank_known_false_unknown_rate"],
        component_decisions["energy_rank_unknown_recall"],
        component_decisions["energy_rank_known_false_unknown_rate"],
        component_decisions["prototype_rank_unknown_recall"],
        component_decisions["prototype_rank_known_false_unknown_rate"],
        component_decisions["mean_rank_fusion_unknown_recall"],
        component_decisions["mean_rank_fusion_known_false_unknown_rate"],
        component_decisions["weighted_rank_fusion_unknown_recall"],
        component_decisions["weighted_rank_fusion_known_false_unknown_rate"],
        int(np.sum(y_pred == open_set_label_id)),
        int(np.sum(fusion_rejects)),
        int(np.sum(weighted_fusion_rejects)),
        int(np.sum(max_fusion_rejects)),
        int(np.sum(proser_rank_rejects)),
        int(np.sum(gen_rank_rejects)),
        int(np.sum(energy_rank_rejects)),
        int(np.sum(prototype_rank_rejects)),
        int(np.sum(proser_rejects)),
        int(np.sum(gen_rejects)),
        int(np.sum(energy_rejects)),
        int(np.sum(energy_dev_rejects)),
        int(np.sum(proto_rejects)),
    )
    return metrics
