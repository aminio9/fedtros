...
echo "📌 [1/3] Running A1-TEACHER (Variational Teacher Ablation)..."
...
echo "📌 [2/3] Running A2-ANCHOR (Retention Anchor & Coverage Ablation)..."
poetry run python scripts/run_study.py A2-ANCHOR \
    --stage main --wandb-mode disabled --seeds 42 \
    --method fedtros_mc --only-missing --output-dir outputs runtime=gpu_fast

echo "📌 [3/3] Running A3-TRANSFER (Knowledge Transfer & Aligner Ablation)..."
