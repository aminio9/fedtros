...
echo "📌 [1/5] Running E5-DATASET (B-NAT, B-TAT, ToN-IoT, CIC-IDS2017) with FedTROS-MC..."
...
echo "📌 [2/5] Running E6-SCALE (Client Scalability: 10, 20, 50, 100 clients)..."
poetry run python scripts/run_study.py E6-SCALE \
    --stage main --wandb-mode disabled --seeds 42 \
    --method fedtros_mc --only-missing --output-dir outputs runtime=gpu_fast

echo "📌 [3/5] Running E7-EFFICIENCY (FL Profiling: Comm / Memory / Runtime)..."
poetry run python scripts/run_study.py E7-EFFICIENCY \
    --stage main --wandb-mode disabled --seeds 42 \
    --method fedtros_mc --only-missing --output-dir outputs runtime=gpu_fast

echo "📌 [4/5] Running E8-LOAO (Leave-One-Attack-Out Cross-Class Validation)..."
poetry run python scripts/run_study.py E8-LOAO \
    --stage main --wandb-mode disabled --seeds 42 \
    --method fedtros_mc --only-missing --output-dir outputs runtime=gpu_fast

echo "📌 [5/5] Running S1-SENSITIVITY (Tempered Aggregation Gamma Sensitivity)..."
