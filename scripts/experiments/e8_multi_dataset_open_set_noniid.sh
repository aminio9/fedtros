...
run_dataset() {
  ...
  invoke_hydra_run \
    ...
    "dataset.preprocessing.output_dir=outputs/e8_${run_slug}/processed" \
    "tracking.run_id=e8_${run_slug}_fmrl_la_seed42"
  ...
  invoke_hydra_run \
    ...
    "dataset.preprocessing.raw_file=$raw_file" \
    "model.num_actions=$num_actions" \
    "dataset.preprocessing.output_dir=outputs/e8_${run_slug}_fedavg/processed" \
    "tracking.run_id=e8_${run_slug}_fedavg_seed42"
  ...
  invoke_hydra_run \
    ...
    "dataset.preprocessing.raw_file=$raw_file" \
    "model.num_actions=$num_actions" \
    "dataset.preprocessing.output_dir=outputs/e8_${run_slug}_fedprox/processed" \
    "tracking.run_id=e8_${run_slug}_fedprox_seed42"
...
if [[ "$ran_any" -eq 0 ]]; then
  echo "E8 scaffold is installed, but no external dataset mappings are finalized yet."
fi
